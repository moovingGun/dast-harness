...
### 스캐너별 주입 수단 (실측: nuclei v3.11.1, nikto 2.6.1 자체 help 출력 기준)

| | nuclei | nikto |
|---|---|---|
| 임의 헤더 | `-H, -header` — **`(cli, file)`**: 파일 경로도 받는다 | `-Add-header` (반복 가능, **argv only**) |
| 전용 인증 시크릿 파일 | **`-sf, -secret-file`** + `-ps, -prefetch-secrets` | 없음 |
| HTTP Basic | 헤더로 구성 | `-id user:pass[:realm]` (**argv only**) |
| 출력 레닥션 | **`-rd, -redact`** — 쿼리 파라미터·요청 헤더·본문에서 주어진 키 마스킹 | 없음 |
| mTLS | `-cc/-ck/-ca` (클라이언트 인증서) | — |

**결론:**

- **nuclei는 argv를 완전히 피할 수 있다.** `-secret-file`(인증 스캔 전용) 또는
  파일 형태의 `-H`를 쓴다. → `injected_via="file"`.
- **nikto는 피할 수 없다.** `-Add-header`·`-id` 모두 argv 전용이라 시크릿이
  `ps` 출력에 노출된다. → `injected_via="argv"`로 **증거에 반드시 남기고**,
  nikto에 크리덴셜을 넘길 때 경고를 발생시킨다.
- 시크릿 파일은 하네스가 `0600`으로 생성하고 스캔 종료 시 반드시 삭제한다
  (`finally` 보장). 임시 파일 경로는 argv에 남지만 그건 시크릿이 아니다.

> **미측정(UNVERIFIED):** nuclei JSONL 레코드가 `-H`로 넘긴 헤더 값을
> `curl-command` 등의 필드에 그대로 싣는지는 이번에 실측하지 않았다. 다만 nuclei가
> `-redact`를 별도 플래그로 제공한다는 사실 자체가 요청 헤더가 출력에 도달함을
> 강하게 시사한다. **샌다고 가정하고 설계한다** (§8). 구현 착수 시 로컬에서
> 1회 실측해 이 항목을 확정할 것.
...
실측 결과 nuclei에는 세 단계가 있다:

| 플래그 | 동작 | 크리덴셜 유출 |
|---|---|---|
| `-dr, -disable-redirects` | 추종 안 함 (현재 불변값) | 불가능 |
| `-fhr, -follow-host-redirects` | **같은 호스트로만** 추종 | 같은 호스트 내로 한정 |
| `-fr, -follow-redirects` | 전부 추종 (`-mr`로 횟수 제한) | **임의 호스트로 유출 가능** |

**제안:**

- **`-fr, -follow-redirects`는 절대 노출하지 않는다.** 크리덴셜을 임의 호스트로
  보내는 스위치이므로 `ScanConfig`에 넣지 않는다.
- 인증이 붙은 스캔에 한해 `-fhr, -follow-host-redirects`를 **옵트인**으로 허용한다
  (`ScanConfig(follow_host_redirects=True)`, 기본 `False`). 크리덴셜은 이미 그
  origin에 바인딩돼 있으므로(§4.3) 동일 호스트 추종은 scope를 넓히지 않는다.
- **단, 검증이 필요하다.** `-fhr`의 "same host" 판정이 포트·스킴까지 보는지, 아니면
  호스트명만 보는지 확인해야 한다. 호스트명만 본다면 `http://h:8080` →
  `https://h:443` 이동이 가능하고, 이는 `AuthScope`(scheme+host+port)보다 느슨하다.
  **확인 전까지는 기본 `False`를 유지한다.**

nikto는 `-followredirects`가 **옵트인**이라 기본값이 이미 안전하다. 그냥 넘기지
않으면 되고, 별도 무력화 플래그가 필요 없다. (nuclei와 달리 "끄는" 게 아니라 "안
켜는" 것이므로, 현재 README의 nikto 하드닝 목록에 추가할 항목은 아니다.)
...
**nuclei는 네이티브 지원이 있다.** `-rd, -redact`에 헤더 키 목록을 넘기면 nuclei가
자기 출력에서 쿼리 파라미터·요청 헤더·본문의 해당 키를 마스킹한다. 크리덴셜을 주입할
때 그 키(`Authorization`, `Cookie`, 커스텀 헤더명)를 **자동으로 `-redact`에 등록한다.**
이게 가장 확실한 방어선이다 — 시크릿이 애초에 nuclei 출력에 안 실린다.

nikto에는 대응 기능이 없으므로 아래 하네스 레벨 스크러버가 유일한 방어선이다.
...
1. **지문 salt** (§4.1) — 평문 SHA-256 앞 8자(실행 간 대조 가능) vs 프로세스별 salt
   (약한 시크릿 보호). 제안: 전자.
2. **`AuthCheck` 미지정 시** (§5) — 경고 후 진행 vs 거부. 제안: 경고 후 진행.
   (검증을 강제하면 초기 도입 마찰이 크다. 단, 증거에 `verified=null`이 남는다.)
3. **`--auth-unsupported`** (§6) — 기본 `fail` vs `skip`. 제안: `fail`.
4. **`-follow-host-redirects` 옵트인 허용 여부** (§7.1) — **가장 중요한 결정.**
   허용하지 않으면 인증 스캔 커버리지가 크게 떨어지고, 허용하면 크리덴셜 전송 범위가
   (동일 호스트 내로만) 넓어진다. 제안: 옵트인으로 허용하되 기본 `False`,
   그리고 nuclei의 "same host" 판정이 포트·스킴까지 보는지 **구현 전 실측 확인**.
5. **nikto에 크리덴셜 허용 여부** (§7) — nikto는 argv 노출을 피할 방법이 없다.
   경고 후 허용 vs 아예 금지. 제안: 경고 후 허용(+ `injected_via="argv"` 증거).

## 11. 구현 전 실측해야 할 항목

설계가 의존하지만 아직 측정하지 않은 것들. 구현 착수 시 로컬에서 확인한다.

- [ ] nuclei JSONL이 `-H` 헤더 값을 출력 필드(`curl-command` 등)에 싣는가, 그리고
      `-redact`가 그걸 실제로 마스킹하는가 (§7, §8)
- [ ] `-secret-file`의 정확한 파일 포맷과 지원 인증 타입 (§7)
- [ ] `-follow-host-redirects`의 "same host" 판정 범위 (호스트명만? 스킴·포트까지?) (§7.1)
- [ ] nikto JSON 결과 파일이 `-id`/`-Add-header` 크리덴셜을 되뱉는가 (§8)
