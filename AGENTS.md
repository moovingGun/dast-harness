...
| 무엇을 하려는가 | 읽을 것 |
|---|---|
| 정찰 / injection / IDOR **에이전트를 만든다** | **[AGENT_GUIDE.md](AGENT_GUIDE.md)** — 이것만 보고 시작할 수 있다 |
...
1. **HTTP 요청을 직접 만들지 않는다.** `requests` / `httpx` / `urllib.request` 금지.
   `dast_harness.agent_kit.AgentHttpClient`만 쓴다. 이유: 에이전트는 타겟의 응답을
